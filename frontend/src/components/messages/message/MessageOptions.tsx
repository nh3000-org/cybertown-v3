...
export function MessageOptions(props: MessageOptionsProps) {
  ...
  return (
    <Dropdown.Root onOpenChange={setOpen}>
      ...
      <Dropdown.Portal>
        <Dropdown.Content className="rounded-lg p-2 shadow-md bg-bg-2 text-fg-2 flex flex-col gap-2 border border-border" side={side} sideOffset={12} align={align} onCloseAutoFocus={e => e.preventDefault()}>
          {isMyMessage && (
            <>
              <Dropdown.Item className="flex gap-3 items-center data-[highlighted]:outline-none data-[highlighted]:bg-highlight px-2 py-1 rounded-md" onClick={handleEdit}>
          ...
          <Dropdown.Item className="flex gap-3 items-center data-[highlighted]:outline-none data-[highlighted]:bg-highlight px-2 py-1 rounded-md" onClick={handleReply}>
