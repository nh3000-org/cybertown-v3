...
export function Reactions(props: Props) {
  ...
  return (
    <div className={cn("flex gap-2 mt-1 text-sm items-center flex-wrap", {
      "justify-end": props.dm !== null && props.message.from.id === currentUser?.id
    })}>
          ...
          <HoverCard.Root key={reaction}>
            <HoverCard.Trigger asChild>
              <button key={reaction} className='text-sm focus:ring-0 flex items-center gap-2 border border-accent/60 bg-accent/20 px-[6px] py-[2px] rounded-md' onClick={() => {
                ws.reactionToMsg(props.message.id, reaction, getParticipantID(props.message, currentUser!) ?? props.dm?.id, props.dm !== null)
              }}>
            ...
            <HoverCard.Portal>
              <HoverCard.Content className='rounded-lg p-3 shadow-md bg-bg-2 text-fg-2 flex flex-col gap-3 border border-border' sideOffset={10}>
                {Object.entries(userMap).map(([key, u]) => {
                  const user = u.id ? u : currentUser?.id! == Number(key) ? currentUser! : props.dm!
