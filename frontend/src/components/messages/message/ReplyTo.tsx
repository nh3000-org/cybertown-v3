...
export function ReplyTo(props: Props) {
  ...
  return (
    <div role="button" onClick={() => scrollToMessage(message.id)} className='flex gap-3 items-start rounded-md border-l-2 border-yellow-500 bg-sidebar p-2 mb-1'>
      {!props.isDM && (
        <img className="w-6 h-6 rounded-md" src={message.from.avatar} referrerPolicy="no-referrer" />
      )}
