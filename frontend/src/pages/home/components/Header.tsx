  ...
  return (
    <header>
      <div className="flex justify-end">
        {user === undefined ? <LoadingIcon className="text-accent/20 fill-accent w-6 h-6 my-2" /> : user ? <UserMenu /> :
          <button onClick={() => {
            setAlert("login", true)
          }} className="bg-accent text-accent-fg h-8 my-1 px-4 rounded-lg rounded-md focus:ring-accent focus:ring-1 focus:ring-offset-2 focus:ring-offset-bg">Login</button>}
      </div>
